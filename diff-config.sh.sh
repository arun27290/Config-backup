...
diff_checksums() {
    ...
    comm -23 <(awk '{print $2}' "${DIFF_DIR}/cs_pre.txt"  | sort) \
              <(awk '{print $2}' "${DIFF_DIR}/cs_post.txt" | sort) \
              > "${DIFF_DIR}/cs_removed_files.txt" 2>/dev/null || true

    # Files only in post (new)
    comm -13 <(awk '{print $2}' "${DIFF_DIR}/cs_pre.txt"  | sort) \
              <(awk '{print $2}' "${DIFF_DIR}/cs_post.txt" | sort) \
              > "${DIFF_DIR}/cs_new_files.txt" 2>/dev/null || true

    # Files in both but hash changed
    ...
    done < <(comm -12 <(awk '{print $2}' "${DIFF_DIR}/cs_pre.txt" | sort) \
                       <(awk '{print $2}' "${DIFF_DIR}/cs_post.txt" | sort))
}
...
SECTIONS=(
    ...
    "16_etc_checksums.txt" "17_pkg_integrity.txt" "18_processes.txt"
    "19_logs.txt"         "20_reboot_history.txt"
)
...
<div class="layout">
  <nav class="nav">
    ...
    <a href="#d-18" onclick="act(this)"><span class="nav-ic">🔄</span>Processes</a>
    <a href="#d-19" onclick="act(this)"><span class="nav-ic">📋</span>Logs</a>
  </nav>
  ...
  <div class="content">
    ...
    <div class="section-card" id="d-18"><div class="sc-hdr" onclick="tog(this)"><div class="sc-left"><div class="sc-ic">🔄</div><div><div class="sc-title">Process Snapshot — Full Diff</div><div class="sc-sub">18_processes.txt</div></div></div><div class="sc-right"><span class="sc-toggle">▼</span></div></div><div class="sc-body">$(html_diff_block "${DIFF_DIR}/diff_18_processes.txt")</div></div>

    <div class="section-card" id="d-19"><div class="sc-hdr" onclick="tog(this)"><div class="sc-left"><div class="sc-ic">📋</div><div><div class="sc-title">Log Baseline — Full Diff</div><div class="sc-sub">19_logs.txt</div></div></div><div class="sc-right"><span class="sc-toggle">▼</span></div></div><div class="sc-body">$(html_diff_block "${DIFF_DIR}/diff_19_logs.txt")</div></div>
