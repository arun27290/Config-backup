...
# ---------------------------------------------------------------------------
# Bash version guard — SLES9/RHEL5 ship Bash 3.x; avoid Bash 4+ syntax
# ---------------------------------------------------------------------------
if [ "${BASH_VERSINFO[0]}" -lt 3 ]; then
    echo "ERROR: Bash 3.0+ required." >&2
    exit 1
fi
...
capture_packages() {
    local f="${SNAPSHOT_DIR}/11_packages.txt"
    {
        ...
        case "$OS_FAMILY" in
            ...
            suse)
                if command -v zypper >/dev/null 2>&1; then
                    zypper patches 2>/dev/null | head -40
        ...
        echo ""
        echo "=== SHARED LIBRARIES (ldconfig) ==="
        ldconfig -p 2>/dev/null | head -80 || echo "ldconfig not available"
        echo ""
        echo "=== REPOSITORIES ==="
        case "$OS_FAMILY" in
            rhel)
                if command -v dnf >/dev/null 2>&1; then dnf repolist 2>/dev/null
                elif command -v yum >/dev/null 2>&1; then yum repolist 2>/dev/null; fi
                ls /etc/yum.repos.d/*.repo 2>/dev/null | while read r; do echo "--- $r ---"; cat "$r"; done
                ;;
            suse)
                command -v zypper >/dev/null 2>&1 && zypper repos 2>/dev/null
                ls /etc/zypp/repos.d/*.repo 2>/dev/null | while read r; do echo "--- $r ---"; cat "$r"; done
                ;;
            debian)
                cat /etc/apt/sources.list 2>/dev/null
                ls /etc/apt/sources.list.d/*.list 2>/dev/null | while read r; do echo "--- $r ---"; cat "$r"; done
                ;;
        esac
    } > "$f" 2>&1
    local pkgcount
    ...
    print_status "Packages & Repos" "OK" "${pkgcount} packages | repos + ldconfig captured"
}
...
capture_package_integrity() {
    local f="${SNAPSHOT_DIR}/17_pkg_integrity.txt"
    {
        echo "=== PACKAGE INTEGRITY VERIFICATION ==="
        echo "Note: This may take a few minutes on large systems."
        echo ""
        case "$OS_FAMILY" in
            rhel|suse)
                echo "--- RPM Verify (rpm -Va) ---"
                rpm -Va 2>/dev/null | head -150 || echo "rpm -Va not available" ;;
            debian)
                echo "--- DPKG Verify ---"
                if dpkg --verify 2>/dev/null; then
                    echo "All verified OK"
                else
                    dpkg --verify 2>/dev/null | head -100
                fi ;;
        esac
    } > "$f" 2>&1
...
# 18 — Processes (NEW)
capture_processes() {
    local f="${SNAPSHOT_DIR}/18_processes.txt"
    {
...
# 19 — Log Baseline
capture_logs() {
    local f="${SNAPSHOT_DIR}/19_logs.txt"
    {
...
capture_reboot_history() {
    local f="${SNAPSHOT_DIR}/20_reboot_history.txt"
    {
        echo "=== REBOOT HISTORY ==="
        last reboot 2>/dev/null | head -20
        echo ""
        echo "=== SHUTDOWN HISTORY ==="
        last -x shutdown 2>/dev/null | head -10 || echo "Not available"
        echo ""
        echo "=== CURRENT UPTIME ==="
        uptime
        echo ""
        ...
        echo ""
        echo "=== /var/log/wtmp summary ==="
        last -F 2>/dev/null | grep -E "reboot|shutdown" | head -20 \
            || last 2>/dev/null | grep -E "reboot|shutdown" | head -20
    } > "$f" 2>&1
    local upt
    upt=$(compat_uptime_pretty)
    print_status "Reboot & Uptime History" "OK" "${upt}"
}
...
<title>${MODE^^} Snapshot — $(hostname -s) — $(date '+%Y-%m-%d')</title>
<style>
:root {
  --bg:       #070d18;
  --bg2:      #0d1526;
...
<div class="hdr">
  <div class="hdr-top">
    ...
    <div class="hdr-meta">
      ...
      <div class="meta-pill"><strong>${OS_FAMILY^^} / ${INIT_SYSTEM}</strong><span>OS Family / Init</span></div>
...
<div class="layout">
  ...
  <nav class="nav">
    ...
    <a href="#s17" onclick="act(this)"><span class="nav-ic">🔍</span>Pkg Integrity</a>
    <div class="nav-grp">Runtime</div>
    <a href="#s18" onclick="act(this)"><span class="nav-ic">🔄</span>Processes</a>
    <div class="nav-grp">Audit</div>
    <a href="#s19" onclick="act(this)"><span class="nav-ic">📋</span>Log Baseline</a>
    <a href="#s20" onclick="act(this)"><span class="nav-ic">♻</span>Reboot History</a>
  </nav>
  ...
  <div class="content">
      ...
      <div class="card-body open">
        <div class="info-grid">
          <div class="info-card">
            ...
            <div class="info-row"><span class="info-key">Family</span><span class="info-val ok">${OS_FAMILY^^}</span></div>
          ...
          <div class="info-card">
            ...
            <div class="info-row"><span class="info-key">Snapshot Mode</span><span class="info-val">${MODE^^}</span></div>
          </div>
        </div>
    ...
    <div class="card" id="s11"><div class="card-hdr" onclick="tog(this)"><div class="card-title-row"><div class="card-ic">📦</div><div><div class="card-title">Packages &amp; Repositories</div><div class="card-sub">${pkgcount} packages · repos · ldconfig · history</div></div></div><div class="card-toggle">▼</div></div><div class="card-body"><pre>$(html_escape "${SNAPSHOT_DIR}/11_packages.txt")</pre></div></div>
    ...
    <div class="card" id="s17"><div class="card-hdr" onclick="tog(this)"><div class="card-title-row"><div class="card-ic">🔍</div><div><div class="card-title">Package Integrity</div><div class="card-sub">RPM verify / DPKG verify</div></div></div><div class="card-toggle">▼</div></div><div class="card-body"><pre>$(html_escape "${SNAPSHOT_DIR}/17_pkg_integrity.txt")</pre></div></div>

    <div class="card" id="s18"><div class="card-hdr" onclick="tog(this)"><div class="card-title-row"><div class="card-ic">🔄</div><div><div class="card-title">Process Snapshot</div><div class="card-sub">ps tree · top CPU/MEM · zombies · open files</div></div></div><div class="card-toggle">▼</div></div><div class="card-body"><pre>$(html_escape "${SNAPSHOT_DIR}/18_processes.txt")</pre></div></div>

    <div class="card" id="s19"><div class="card-hdr" onclick="tog(this)"><div class="card-title-row"><div class="card-ic">📋</div><div><div class="card-title">System Log Baseline</div><div class="card-sub">syslog · dmesg · journal · auth · boot</div></div></div><div class="card-toggle">▼</div></div><div class="card-body"><pre>$(html_escape "${SNAPSHOT_DIR}/19_logs.txt")</pre></div></div>

    <div class="card" id="s20"><div class="card-hdr" onclick="tog(this)"><div class="card-title-row"><div class="card-ic">♻</div><div><div class="card-title">Reboot &amp; Uptime History</div><div class="card-sub">last reboot · shutdowns · uptime</div></div></div><div class="card-toggle">▼</div></div><div class="card-body"><pre>$(html_escape "${SNAPSHOT_DIR}/20_reboot_history.txt")</pre></div></div>

  </div><!-- /content -->
</div><!-- /layout -->

<div class="footer">
  <span>Config Snapshot Tool v2.0 &nbsp;·&nbsp; $(hostname -f 2>/dev/null||hostname) &nbsp;·&nbsp; ${MODE^^} &nbsp;·&nbsp; $(date '+%Y-%m-%d %H:%M:%S')</span>
...
main() {
    ...
    echo -e "  ${DIM}──────────────────────────────────────────────────────────────────${RESET}"
    echo ""
    echo -e "  ${BOLD}${mode_color}✔  ${MODE^^}-REBOOT SNAPSHOT COMPLETE${RESET}"
    echo ""
